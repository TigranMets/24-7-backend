...
const ViewsProvider = require('../provider/ViewsProvider')
...
class NewsService extends BaseService {
    ...
    async createNews(req) {
        try {
            ...
            // remove views record if created before prev month
            const oldNews = await this.viewsProvider.getOlderNews(getLastMonthDate())

            for (let i = 0; i < oldNews.length; i++) {
                await this.viewssModel.destroy({
                    where: {newsId: oldNews[i].newsId}
                })
            }
            return this.response({
    ...
    async moreReadableNews(req) {
        try {
            const {date} = req.query;

            let startDate = getPreviousDay()
            ...
            return this.response({
                data: await this.viewsProvider.filterViews(startDate, endDate)
            })
